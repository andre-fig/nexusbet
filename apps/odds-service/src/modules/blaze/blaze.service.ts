...
export class BlazeService implements ProviderRuntime {
  ...
  constructor(
    ...
    @Inject(BlazeCollector) private readonly collector: BlazeCollector,
  ...
  private async scan() {
    try {
      ...
      const directory = this.config.settings.blazeInboxDir;
  ...
  health() {
    return {
      ...
      authenticated: false,
      enabled: this.config.settings.blazeEnabled,
      transport: "http",
  ...
  readEvents(games: Esport[]) {
    ...
    if (
      ...
      throw new StaleDataError("Missing or stale Blaze listing");
  ...
  readDetail(id: string, games: Esport[]) {
    ...
    if (
      ...
      throw new StaleDataError("Missing or stale Blaze detail");
