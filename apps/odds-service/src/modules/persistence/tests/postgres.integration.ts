...
test("FaZe Clan and Nemiga persist one StarLadder/Stake Ranked match with aliases", async () => {
  const startsAt = "2026-10-01T17:30:00.000Z";
  const make = (
  ...
  assert.equal(await database.db.canonicalEvent.count(), 1);
  const canonical = await database.db.canonicalEvent.findFirstOrThrow();
  assert.equal(canonical.teamA, "FaZe Clan");
  assert.equal(canonical.teamB, "Nemiga");
  assert.equal(canonical.tournament, "starladder ranked");
  assert.equal(
    (
      await database.db.teamAlias.findUniqueOrThrow({
        where: { esport_alias: { esport: "cs2", alias: "faze clan" } },
      })
    ).canonicalName,
    "faze",
  );
  assert.equal(
    (
      await database.db.tournamentAlias.findUniqueOrThrow({
        where: { esport_alias: { esport: "cs2", alias: "stake ranked" } },
      })
    ).canonicalName,
    "starladder ranked",
  );
