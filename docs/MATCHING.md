...
Em CS2, `apogee` e `betclic` usam a chave `betclic apogee`; `Betclic Apogee Esports` chega à mesma chave pela remoção do sufixo. `Astral` e `ASTRAL Esports` usam a chave `astral`. Quando um grupo é aprovado, o repository persiste os aliases semânticos observados. Os nomes gravados no evento canônico para essas equipes são `Betclic Apogee Esports` e `ASTRAL Esports`; a chave de matching permanece normalizada.

Também em CS2, `FaZe Clan` usa a chave `faze` por alias revisado, sem remover `Clan` dos demais nomes; `Nemiga` usa a chave `nemiga`. Os nomes canônicos são `FaZe Clan` e `Nemiga`. `Stake Ranked` e `StarLadder Ranked` usam a competição canônica `starladder ranked`; o alias de torneio observado é persistido quando o grupo passa na validação.

Aliases de competição atuais incluem as variantes de StarLadder/StarSeries em CS2, `cs2 european pro league` → `european pro league`, `cct challengers sa` → `cct south america challenger`, `pulse beat ii` → `stake pulse beat`, `lol lec summer playoffs` → `lec`, `lol cblol split 2 playoffs` → `cblol`, e variantes VCT Champions → `champions`. Um prefixo explícito do esporte seguido de `-`, `:`, `|` ou `/` é removido da chave do torneio; fase, temporada e número continuam na chave. As tabelas executáveis ficam em `modules/matching/team-aliases.ts` para equipes e `shared/utils/names.ts` para torneios; aliases confirmados ficam em `team_aliases` e `tournament_aliases`. Um alias de time novo requer grupo não ambíguo, mesmo adversário e torneio normalizados, horário a até cinco minutos e abreviação com sufixo idêntico, uma única flexão de plural ou uma única inserção/remoção/substituição em um token de pelo menos cinco letras. Por exemplo, `EAC Extra` → `EA Copenhagen Extra`, `Semente do Mal` → `Sementes do Mal` e `Johny Speeds` → `Johnny Speeds`. A similaridade ortográfica é apenas sinal auxiliar: sozinha não une partidas. A variante do torneio só é gravada quando uma equivalência revisada é confirmada por esse grupo. Não há distância de edição geral.
