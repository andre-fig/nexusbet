...
A função de limpeza aplica NFKD, remove marcas de acentuação, converte para minúsculas, substitui pontuação por espaços e compacta espaços. Só na chave de matching, remove o prefixo `team ` e um sufixo final ` esports`, ` esport`, ` e-sports` ou ` gaming`, depois compacta a borda novamente. Como pontuação já foi normalizada, `e-sports` é reconhecido como `e sports`. Em Valorant, um marcador final `(F)` também é removido antes da limpeza, com ou sem espaço anterior; um `F` sem parênteses ou no meio do nome permanece. Nomes raw e os campos normalized próprios do provider não são reescritos.

Depois da regra de borda, aliases explícitos por esporte cobrem apenas equivalências semânticas: CS2 inclui `navi` → `natus vincere`, `33` → `team 33` e `l g` → `leo team`; LoL inclui `vivo keyd stars` → `keyd stars`, `movistar koi` → `koi`, `9z globant` / `9z team` → `9z`; Valorant inclui `fennel f` → `fennel gc`. Essa equivalência revisada prevalece para `FENNEL (F)` antes de retirar o marcador final, evitando confundir `FENNEL GC` com um `FENNEL` sem marcador. `academy`, `junior`, `young`, `gc`, `female`, `ex`, `fe` e `youth` permanecem na chave: `Nexus Gaming` e `Nexus` podem unir, mas `Nexus Academy Gaming` não vira `Nexus`. A regra não usa distância de edição nem fuzzy matching; preserve testes de colisão ao acrescentar novos aliases.
