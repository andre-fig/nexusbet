...
`MarketAnalytics` contém `category`, `mapNumber`, `marketIds`, `bestPrices[]`, `outliers[]`, `valueBets[]` e `arbitrage` (null quando não existe). BEST PRICE traz provider, seleção, odd, exibição e próxima melhor odd; empates são resolvidos pelo nome do provider. OUTLIER traz direção up/down, odd, mediana e desvio percentual assinado. VALUE_BET traz fair odd, edge e contagem de providers. ARBITRAGE traz inverseSum, margem percentual, legs, distribuição de R$100 e retorno teórico. Os tooltips contextuais são produzidos no backend e apenas exibidos pelo React. A análise exige um vínculo canônico `matched` ou `partial`, ao menos dois providers válidos, mesma categoria/mapa/período/linha e dois resultados que correspondam aos times canônicos. Mercados ambíguos, incompletos, suspensos, live, stale, disabled ou com odds inválidas não entram. Outlier exige pelo menos três providers válidos e usa `ODDS_OUTLIER_THRESHOLD_PERCENT=10` por padrão. VALUE_BET exige matching `matched` com confidence >= 0,8, três providers válidos e `VALUE_BET_MIN_EDGE_PERCENT=5` por padrão. Cada mercado de dois resultados é de-vigged pela soma das probabilidades implícitas; a mediana das probabilidades normalizadas por seleção define a fair odd. A função de de-vig também suporta três resultados completos, mas o sinal atual permanece nos mercados de vencedor de dois resultados com seleção canônica segura. Best price, value bet e arbitragem não criam issues; outlier é sinal de divergência na leitura, sem afirmar erro do provider e sem aumentar o contador persistido de issues.

No odds-monitor, a própria odd recebe a cor e o tooltip contextual em inglês, com foco por teclado: Best price `#5B8CFF`, Outlier `#E4A84B`, Arbitrage `#37C978`. A prioridade visual é Arbitrage > Outlier > Best price quando a mesma seleção tem mais de um sinal. Não há ícones ou badges de analytics ao lado da odd; o React apenas escolhe qual resultado da API mostrar, sem recalcular as métricas.
...
Qualidade dos mercados: `MARKET_INCOMPLETE` é uma issue persistida `warning` somente quando um mercado `match_winner` ou `map_winner` foi observado com menos de duas odds válidas para um evento de dois participantes. Uma captura que não contém um mercado não abre essa issue; o provider retorna `marketAvailability.matchWinner/map1Winner/map2Winner/map3Winner: "unavailable"` para o mercado ausente, sem prejudicar seus outros mercados. Um mercado ou seleção observado que ultrapassou `MAX_AGE_SECONDS` retorna `status: "stale"`; a última odd continua na resposta para diagnóstico, mas não entra nas análises. `STALE` aparece como issue virtual `warning` na Monitor API enquanto o mercado listado e ativo permanece expirado. É calculada nos GETs, porque o TTL pode expirar sem uma publicação; não cria linha em `data_issues` ou altera snapshots. O contador de Needs attention inclui essas issues virtuais, e `attentionOnly=true` também encontra eventos com mercado stale. Mercados suspensos ou live ficam fora desse alerta de stale. O SSE emite apenas invalidações `market.stale`/`market.refreshed` após a transição, inclusive quando o TTL vence entre publicações; a UI busca os dados novamente via REST.
...
- Matching usa vínculos/decisões existentes. A cobertura do monitor é calculada por esporte contra providers habilitados e ativos no runtime; um provider ativo continua esperado quando fica stale ou unavailable, para que a falha permaneça visível. Um provider `degraded` com zero eventos listados não entra na cobertura até voltar a ter eventos ou sair desse estado. `partial` representa ao menos dois providers associados, mas menos que `expectedProviderCount`; com menos de dois ativos o status é `not_applicable`, fica fora de matched/partial/unmatched e não aparece em Needs attention. No Railway, onde Bet365/Betano são intencionalmente excluídos sem `local-cdp`, os três providers HTTP produzem `expectedProviderCount:3` quando não estão `degraded` com zero eventos; snapshots antigos dos providers desabilitados continuam visíveis como `Disabled in this runtime`, mas não entram no denominador. Isso não é quorum de pricing. Confidence é o mínimo das decisões associadas, não uma probabilidade de vitória.
