...
- Matching usa vínculos/decisões existentes. A cobertura é calculada por esporte contra providers habilitados e operacionais com listagem fresca naquele contexto. `partial` representa ao menos dois providers associados, mas menos que `expectedProviderCount`; com menos de dois elegíveis o status é `not_applicable`, fica fora de matched/partial/unmatched e não aparece em Needs attention. No Railway, onde Bet365/Betano são intencionalmente excluídos sem `local-cdp`, três feeds HTTP válidos produzem `expectedProviderCount:3`; snapshots antigos dos providers desabilitados continuam visíveis como `Disabled in this runtime`, mas não entram no denominador. Isso não é quorum de pricing. Confidence é o mínimo das decisões associadas, não uma probabilidade de vitória.
